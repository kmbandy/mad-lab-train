...
OUT_PATH = Path("/home/kmbandy/mad-lab-mcp/datasets/tool_calls_b.jsonl")
SEED = 137  # different seed for diversity
random.seed(SEED)

# Process B: only generate the 4 unstarted categories
_B_CATEGORIES = {"exec_code", "web_search", "multi_memory", "multi_task_github"}
...
def main():
    ...
    total_target = 0
    for s in SCENARIOS:
        total_target += n_per_scenario

    print(f"Generating ~{total_target} tool-calling examples → {OUT_PATH}")
    OUT_PATH.parent.mkdir(parents=True, exist_ok=True)

    # Resume: count already-generated examples per category
    ...
    with OUT_PATH.open("a") as f:
        for scenario in [s for s in SCENARIOS if s["category"] in _B_CATEGORIES]:
            cat = scenario["category"]
